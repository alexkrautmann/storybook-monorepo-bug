...
if [ ! -z "$GOOGLE_SERVICES_PLIST" ]; then
  echo $GOOGLE_SERVICES_PLIST | base64 --decode > "$APPCENTER_SOURCE_DIRECTORY/ios/chatapp/GoogleService-Info.plist"
fi
...
if [ ! -z "$IOS_CODEPUSH_APPID" ]; then
  ...
  if [[ "$TRIGGER_IOS" != "" ]]; then
    ...
    code-push release-react $IOS_CODEPUSH_APPID ios --outputDir ./build --plistFile ./ios/chatapp/Info.plist --description "$COMMIT_MESSAGE"
